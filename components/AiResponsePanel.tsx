...
const AiResponsePanel: React.FC<AiResponsePanelProps> = ({ response, isTyping, onFixAuth }) => {
  ...
  return (
    <div className="relative w-full max-w-2xl mx-auto mb-8 animate-in fade-in duration-700">
      <div className={`p-6 md:p-8 rounded-[3rem] bg-white/95 dark:bg-white/5 backdrop-blur-[40px] border ${isAuthError ? 'border-red-500/60' : 'border-black/10 dark:border-white/10'} shadow-[0_32px_128px_rgba(0,0,0,0.1)] dark:shadow-[0_32px_128px_rgba(0,0,0,0.6)] transition-all duration-700 overflow-hidden`}>
        ...
        {content && (
          <div className="space-y-8">
            <div className={`prose dark:prose-invert prose-p:leading-relaxed ${isAuthError ? 'prose-p:text-red-700 dark:prose-p:text-red-300' : 'prose-p:text-black dark:prose-p:text-gray-100'} prose-p:font-normal prose-p:text-lg animate-materialize`} dir="auto">
            ...
            {isAnimationComplete && !isAuthError && (
              <div className="flex items-center justify-end gap-3 pt-6 mt-4 border-t border-black/10 dark:border-white/5 animate-in fade-in slide-in-from-bottom-2 duration-1000">
                ...
                <button 
                  ...
                  disabled={isSpeaking || !!ttsAudioUrl}
                  className={`p-3 rounded-2xl bg-black/5 dark:bg-white/5 backdrop-blur-md border border-black/10 dark:border-white/10 shadow-lg transition-all active:scale-95 flex items-center gap-2 ${isSpeaking ? 'text-blue-600 animate-pulse' : (ttsAudioUrl ? 'text-blue-500 opacity-50 cursor-default' : 'text-slate-800 dark:text-gray-500 hover:text-blue-700 dark:hover:text-blue-400')}`}
                  title="Read Aloud"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z"/></svg>
                  <span className="text-[9px] uppercase tracking-widest font-bold">شنیدن</span>
                </button>
            ...
            {ttsAudioUrl && isAnimationComplete && (
            ...
            {response?.mediaUrl && isAnimationComplete && (
              <div className="mt-6 space-y-6">
                ...
                {response.mediaType === 'image' && (
                  <div className="p-8 bg-white/70 dark:bg-white/5 backdrop-blur-[20px] rounded-[2.5rem] border border-black/10 dark:border-white/10 space-y-10 animate-in fade-in slide-in-from-top-4 duration-1000 shadow-xl">
